...
def apply_cheat(i, j, step, field):           #尝试作弊  
	cheats = {}

	neighbours_old = get_neighbours(i, j)     #找出它的相邻格位置
	for cheat_length in range(2, 21):         #作弊的距离更远了，最大作弊20格
		neighbours_new = set()
...
for step in range(race_time):                           #从起点开始，逐步尝试作弊
	...
	all_cheats.update(cheats)                           #将作弊步骤添加到总列表中 

times = [time for _, time in all_cheats.items()]        #找出all_cheats里面所有的节省步长
