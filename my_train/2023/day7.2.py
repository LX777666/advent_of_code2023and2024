...
def card_type(card):   #计算判断card的类型，并返回对应的等级
   ...
   if "J" in card:    #计算J的个数
     ...
     for i in range(num_J):
      ...
      num_J=0
   if num_J !=5: #如果全是J则直接定义list_n=[5]，否则进行处理计算J牌后的list_n
       while card:         #直到card为空
          c=card[0]
       ...
       max_n=max(list_n)   #找出list_n里面的最大值，加上J的个数，重新定义list_n
       for i in  range(len(list_n)):
         if list_n[i]==max_n:
              list_n[i]=list_n[i]+num_J
              break
   else:
       list_n=[5]
...
def sort_card(cards):                             #对cards进行冒泡排序，小的在前面
    ...
    return cards
with open("C:\\Users\\sage\\Desktop\\train\\input2\\input7.txt", "r") as f:
